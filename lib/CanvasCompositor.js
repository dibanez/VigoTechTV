...
function CanvasCompositor() {
    ...
    // layout: { fullCanvas: bool, x, y, width, height }
    ...
    function draw() {
        ...
        for (var j = 0; j < entries.length; j++) {
            ...
            var w = entries[j].width || entries[j].latestFrame.displayWidth || 320;
            var h = entries[j].height || entries[j].latestFrame.displayHeight || 240;
            try {
                context.drawImage(entries[j].latestFrame, entries[j].x, entries[j].y, w, h);
                drewSomething = true;
            } catch (e) {
